...
export async function addNewRecordAll(store:Record<string, any>, attributeDescriptions:TableAttribute[], table_name:string){
    const data = unwrap(store)


    const dataPredefinedAttributes = attributeDescriptions.reduce((acc: Record<string, any>, name: string) => {
        acc[name] = data[name]; // Adds a key-value pair to the accumulator object
        return acc;
    }, {} as Record<string, any>);
    await addNewRecord({custom: dataCustomAttributes, predefined: dataPredefinedAttributes},  table_name )
}