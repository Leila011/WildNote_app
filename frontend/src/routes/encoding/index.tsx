...
import { Experiment, Subject, TableAttribute, TableAttributeValue } from "~/types/db";
  ...
  const params = useParams();
    ...
    function fetcherSubject(props: { experimentId: number}) {
      return props.experimentId
        ? fetchSubjects(props.experimentId)
        : undefined;
    }
      ...
      <main class="text-center mx-auto text-gray-700 p-4">
            <div>
              ...
              <DropdownMenu>
                ...
                >
                  <div class="flex-grow text-left">{!experiment() ? "Pick your experiment": experiment()!.name}</div>
                  <IconChevronDown />
                </DropdownMenuTrigger>
                <DropdownMenuContent>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger
                  ...
                  class={`bg-card text-card-foreground border rounded-md h-10 pl-2 justify-start  w-full`}
                >
                  <div class="flex-grow text-left">{!subject() ? "Pick your Subject": subject()!.name}</div>
                  <IconChevronDown />
                </DropdownMenuTrigger>
                <DropdownMenuContent>
                  <For each={subjects()}>
